...
class ZoneDetailDialog(QDialog):
    def __init__(self, zone, node_store, crop_registry, bus, parent=None):
        ...
        self.setWindowTitle(self.tr("Cấu hình khu tưới"))
        self.setModal(True)
        self.resize(700, 520)
        ...
        assignment_box = QGroupBox(self.tr("Gán thiết bị"))
        ...
        self.add_sensor_node_btn = QPushButton(self.tr("Thêm cảm biến"))
        ...
        self.add_pump_node_btn = QPushButton(self.tr("Thêm bơm"))
        ...
        sensor_row.addWidget(QLabel(self.tr("Cảm biến:")))
        sensor_row.addWidget(self.sensor_node_combo, 1)
        sensor_row.addWidget(self.add_sensor_node_btn)
        assignment_layout.addLayout(sensor_row)
        ...
        assignment_layout.addWidget(sensor_note)
        ...
        assignment_layout.addLayout(pump_node_row)

        assignment_layout.addWidget(QLabel(self.tr("Bơm:")))
        self.pump_combo = QComboBox()
        self.pump_combo.currentIndexChanged.connect(
            self._refresh_irrigation_preview
        )
        assignment_layout.addWidget(self.pump_combo)
        ...
        assignment_layout.addLayout(crop_row)
        ...
        assignment_layout.addLayout(mode_row)

        help_lbl = QLabel(
            self.tr(
                "Mẹo: Nếu danh sách node trống, hãy vào Thêm khu để đăng ký node trước."
            )
        )
        help_lbl.setObjectName("SensorLabel")
        help_lbl.setWordWrap(True)
        assignment_layout.addWidget(help_lbl)

        action_title = QLabel(self.tr("Tác vụ"))
        action_title.setObjectName("ZoneSetupTitle")
        assignment_layout.addWidget(action_title)

        actions_row = QHBoxLayout()
        self.water_now_btn = QPushButton(self.tr("TƯỚI NGAY"))
        self.water_now_btn.setObjectName("ZoneInlineButton")
        self.water_now_btn.clicked.connect(self._water_now)

        self.remove_zone_btn = QPushButton(self.tr("XÓA KHU"))
        self.remove_zone_btn.setObjectName("ZoneInlineButton")
        self.remove_zone_btn.clicked.connect(self._mark_remove_zone)

        actions_row.addWidget(self.water_now_btn)
        actions_row.addWidget(self.remove_zone_btn)
        assignment_layout.addLayout(actions_row)

        layout.addWidget(assignment_box, 1)

        btns = QHBoxLayout()
        btns.addStretch(1)

        close = QPushButton(self.tr("Đóng"))
        save = QPushButton(self.tr("Lưu"))
        ...
        self._reload_node_combos(
            select_sensor_node_id=zone.sensor_node_id,
            select_pump_node_id=zone.pump_node_id,
        )
    ...
    def _on_pump_node_changed(self):
        self._rebuild_pump_combo(self._current_pump_node())

    def _rebuild_pump_combo(self, node):
        ...
        if node.id == self.zone.pump_node_id and self.zone.pump_idx is not None:
            current = self.pump_combo.findData(self.zone.pump_idx)
            if current >= 0:
                self.pump_combo.setCurrentIndex(current)
    ...
    def on_save(self):
        self.zone.name = self.name_edit.text().strip() or self.tr("Khu tưới")

        sensor_node_id = self.sensor_node_combo.currentData() or ""
        pump_node_id = self.pump_node_combo.currentData() or ""

        self.zone.sensor_node_id = sensor_node_id
        self.zone.pump_node_id = pump_node_id
        self.zone.sensor_indices = []

        if not sensor_node_id:
            self.zone.sensor_cache = {}

        if not pump_node_id:
            self.zone.pump_idx = None
            self.node_store.save()
            self.accept()
            return
